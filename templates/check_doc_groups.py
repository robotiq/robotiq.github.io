...
def main():
    ...
    for ns_file in sorted(args.xml_dir.glob("namespace_robotiq*.xml")):
        # Doxygen encodes nested namespaces with "_1_1" between segments
        # (e.g. Robotiq::detail::Foo -> namespace_robotiq_1_1detail_1_1foo),
        # so this glob also matches every *nested* namespace this way --
        # not just legitimate public ones like Robotiq::units or
        # Robotiq::profiles, but also Robotiq::detail, this codebase's
        # internal-implementation convention for \snippet-tagged doc-comment
        # example code (see the docs site's own equivalent exclusion in
        # discoverNamespaceXmlPaths, sync-external-docs.js). That one must
        # stay excluded -- flagging it just suggests giving deliberately
        # internal-only code a public \ingroup to make the warning go away.
        # Only that one exact segment is skipped: this must NOT become "skip
        # every nested namespace", which would also stop checking the
        # legitimate ones -- units.hpp and DeviceProfile (see the module
        # docstring above) are exactly the kind of gap this script exists to
        # catch, and both live in nested namespaces.
        if "detail" in ns_file.stem.split("_1_1")[1:]:
            continue
