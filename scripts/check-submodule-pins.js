...
// while prototyping (found while building this script: 2f85_cpp's entry
// currently declares `documentation-update`, not `main`) and could be
// forgotten the same way the pin could. Trusting it as ground truth would
// make this check blind to that exact case — asking the remote directly
// means neither the pin nor the declared branch can quietly drift without
// this catching it.
//
...
for (const [submodule, { repoUrl, declaredBranch }] of submodules) {
  ...
  if (declaredBranch !== defaultBranch) {
    console.error(
      ...
      `${repoUrl}'s actual default branch is '${defaultBranch}'. If this isn't deliberate (tracking a stable ` +
      `non-default branch on purpose), external-jobs.js is still pointed at a WIP branch — change it back ` +
      `to '${defaultBranch}'.`
    );
    failures += 1;
  }
