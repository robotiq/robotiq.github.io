...
//  - one table per category (SDK, ROS2, ROS1, Simulation, Other) in
//    docs/intro.mdx, all products x tools in that category
//  - the same SDK / Simulation / Other tables, single-row versions
...
// between (SDK/, ROS/, Simulation/) are just containers — except ROS/
...
const CATEGORIES = ['SDK', 'ROS2', 'ROS1', 'Simulation', 'Other'];
...
const PRODUCT_PAGE_CATEGORIES = ['SDK', 'Simulation', 'Other'];
...
// docs/contribute/how-it-works.mdx (e.g. SDK/C++/docs/, SDK/C++/API/).
...
// A guide folder's own sibling files (e.g. SDK/C++/docs/'s
...
function ensureSubpagesBlock(indexPath) {
  ...
  // A subpage that's itself a guide folder (e.g. SDK/C++/docs/, holding
...
// --- each product's own index.mdx: SDK / Simulation / Other tables,
